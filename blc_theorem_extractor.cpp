...
int main() {
    fs::create_directory("theorems");
    
    int target_len;
    if (!(std::cin >> target_len) || target_len <= 0) return 1;

    std::string filename = "blc_dumps/" + std::to_string(target_len) + ".bin";
    ...
    fs::path theorem_path = fs::path("theorems") / (std::to_string(target_len) + ".txt");
    std::ofstream outfile(theorem_path, std::ios::binary);
    ...
    while (true) {
        ...
        if (ast && parse_idx == prog_bits.size()) {
            ...
            if (success && inferred_type) {
                ...
                outfile << "Proof[" << prog_bits << "]\u22A2Theorem:" << theorem << "\n" << "Witness[" << output << "]\n";
            }
        }
    }

    outfile << "\n----------------------------------------\n";
    outfile << "Layer Synthesis Report:\n";
    outfile << "Total Evaluated Programs: " << total_computed << "\n";
